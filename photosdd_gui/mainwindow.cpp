...
void MainWindow::initSignalSlots() {
	...
	// About Boxes in Menu
	connect(ui->actionAbout_PhotosDD_GUI, SIGNAL(triggered()), this,
			SLOT(aboutPhotosDD_GUI()));
	connect(ui->actionAbout_PhotosDD, SIGNAL(triggered()), this,
			SLOT(aboutPhotosDD()));
...
void MainWindow::setSaveToDir() {
	...
	QString dir = QFileDialog::getExistingDirectory(this,
			"Please select a directory.", ui->SaveToDirLineEdit->text());
...
void MainWindow::download() {
	...
	for (int i = 0; i < plugins.count(); i++) {
		connect(photosDD->getPlugins().at(i), SIGNAL(progress(int)),
				ui->pluginProgressBar, SLOT(setValue(int)));
...
void MainWindow::copyImages(QStringList list) {
	foreach(QString file, list)
		{
			qDebug("[MainWindow] copies %s to %s", qPrintable(file),
					qPrintable(ui->SaveToDirLineEdit->text()));
...
void MainWindow::on_DownloadButton_clicked() {
	...
	if (dir.exists()) {
		...
		QMessageBox::information(0, tr("Information"),
				tr("Direcory to save to does not exist"));
	}
}
...
void MainWindow::loadConfig() {
	QSettings settings(QCoreApplication::organizationName(),
			QCoreApplication::applicationName());
	ui->SaveToDirLineEdit->setText(
			settings.value("SaveToDir", QDir::homePath()).toString());
}
