...
Picasa::Picasa(QWidget *parent) :
	QWidget(parent), ui(new Ui::Picasa) {
	// Localization
	...
	connect(ui->webView, SIGNAL(loadProgress(int)), this,
...
void Picasa::download() {
	...
	foreach (QWebElement el, col)
		{
			...
			if ((url.contains("s128") || url.contains("s144-c"))
	...
	for (int i = 0; i < urlList.size(); i++) {
		...
		if (PhotosDD::download(urlList.at(i), destination)) {
			...
			qDebug("[Picasa] not downloaded file: %s",
