...
bool Downloader::downloadFile(QString url, QString folder) {
	qDebug("[Downloader] downloads %s to %s", qPrintable(url),
			qPrintable(folder));
	...
	if (!dir.exists()) {
		if (!dir.mkdir(dir.path())) {
			qDebug("[Downloader] Could not create directory %s",
					qPrintable(folder));
		} else {
	...
	QString localFileName = folder + QDir::separator()
	...
	if (ret == false)
		qDebug("[Downloader] file error: %s", qPrintable(f.errorString()));
	return ret;
}

bool Downloader::downloadHttp(QUrl url, QString folder) {
	qDebug("[Downloader] downloads %s to %s", qPrintable(url.toString()),
			qPrintable(folder));
