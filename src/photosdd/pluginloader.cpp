...
void PluginLoader::loadConfig(){
    ...
    activatePlugin("File");
    // load from settings.
    QSettings settings(PhotosDD::organizationName(), PhotosDD::applicationName());
    settings.beginGroup(PhotosDD::applicationName());
    settings.beginGroup("plugins");
    qDebug("[PluginLoader] settings path: %s / %s / %s / %s", qPrintable(PhotosDD::organizationName()),
           qPrintable(PhotosDD::applicationName()),qPrintable(PhotosDD::applicationName()),
           qPrintable(settings.group()));

    QStringList list;
    list = settings.value("ActivatedPlugins", QStringList()).toStringList();
    activatePlugins(list);
    qDebug("[PluginLoader] loaded config for %s",qPrintable(PhotosDD::applicationName()));
}
...
void PluginLoader::saveConfig(){
    ...
    for( int i = 0; i < this->getActivatedPlugins().size(); i++){
        stringList.append(this->getActivatedPlugins().at(i)->getName()+" "
                          +this->getActivatedPlugins().at(i)->getVersion());
    }
    QVariant list;
    list.setValue(stringList);
    settings.setValue("ActivatedPlugins", list);
    qDebug("[PluginLoader] saved config for %s",qPrintable(PhotosDD::applicationName()));
}
...
void PluginLoader::loadPlugins(){
    ...
    foreach (QString fileName, pluginsDir.entryList(QDir::Files)) {
        ...
        if (plugin) {
            // loads Plugins
            PhotosDDPlugin *plugin_ = qobject_cast<PhotosDDPlugin *>(plugin);
            if (plugin_){
                addPlugin(plugin_);
            }else{
                delete plugin_;
